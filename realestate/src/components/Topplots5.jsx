...
const Topplots5 = () => {
    ...
    return (
        <section className='h-fit'>
            <div className="md:left-[33%] md:top-[40%] top-[35%] left-[35%] rotate-270 absolute font-bold">
                <p className='md:text-[16px] text-[12px]'>9.00 M WIDE ROAD</p>
            </div>
            <div className="flex md:pr-10 pr-10 ">
                <div>
                    {column1.map(({ num, bg }) => (
                        <div key={num} className={`${bg} md:w-10 w-7  h-10 md:border-1 border-1 border-black flex justify-center items-center cursor-pointer`} onClick={() => handleOpenForm(num)}>
                ...
                <div>
                    {column2.map(({ num, bg }) => (
                        <div key={num} className={`${bg} md:w-13 w-9 h-[16.6%] md:border-1 border-1 border-black flex justify-center items-center cursor-pointer`} onClick={() => handleOpenForm(num)}>
                             <p className="text-pink-500 md:text-sm text-[10px]">{num}</p>
                        </div>
                    ))}
                </div>
